...
import (
	...
	"github.com/momentag/ebliss/sdk/helpers/strutils"
)
...
func ParseCiphers(cipherStr string) ([]uint16, error) {
	suites := []uint16{}
	ciphers := strutils.ParseStringSlice(cipherStr, ",")
...
func SetupTLSConfig(conf map[string]string, address string) (*tls.Config, error) {
	...
	if tlsSkipVerify != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("failed parsing tls_skip_verify parameter: %v", err)
		}
